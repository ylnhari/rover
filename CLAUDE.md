...
## Context
Default branch **master**, remote `github.com/ylnhari/rover`. Machine-local details (the address the live instance binds, ports, where the secret lives) belong in the gitignored `CLAUDE.local.md`, never here.
...
```
See `CLAUDE.local.md` for how to restart the live tailnet instance and where `ROVER_SECRET` is stored on this machine.
